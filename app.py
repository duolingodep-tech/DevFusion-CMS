...
def guard():
    setup()
    if request.endpoint in ["admin_login", "static"] or (request.endpoint and request.endpoint.startswith("hidden_admin")):
        return
    if setting("maintenance") == "on" and not admin():
...
def index():
    ...
    prices=[
        ("3D-модели",[("Простая модель","1 000–2 000 Kč"),("Средняя модель","3 000–10 000 Kč"),("Средне-сложная","10 000–20 000 Kč"),("Сложная","20 000–40 000 Kč"),("Очень тяжёлая","30 000–100 000 Kč")]),
        ("Сайты",[("Сайт-визитка","5 000–10 000 Kč"),("Сайт 1–5 страниц","10 000–20 000 Kč"),("Форум / крупный сайт","20 000–40 000 Kč"),("Магазин / кабинет","40 000–100 000 Kč")]),
        ("Android",[("Простое устройство","5 000–10 000 Kč"),("Сложное устройство","10 000–12 000 Kč")])
    ]
...
def add_log():
    ...
    db.session.add(SiteLog(title=request.form.get("title","Новость"), text=request.form.get("text",""), filename=filename, media_type=media_type))
    db.session.commit()
    return redirect(url_for("admin_panel"))
...
def add_ad():
    ...
    db.session.add(AdBlock(title=request.form.get("title","Реклама"), text=request.form.get("text",""), link=request.form.get("link",""), filename=filename, media_type=media_type, enabled=True))
    db.session.commit()
    return redirect(url_for("admin_panel"))
...
def add_block():
    if not admin(): return redirect(url_for("admin_login"))
    db.session.add(CustomBlock(title=request.form.get("title","Новая плитка"), text=request.form.get("text",""), icon=request.form.get("icon","✨"), enabled=True))
    db.session.commit()
    return redirect(url_for("admin_panel"))
...
def add_page():
    ...
    db.session.add(CustomPage(slug=slug, title=request.form.get("title","Новая страница"), body=request.form.get("body",""), enabled=True))
    db.session.commit()
    return redirect(url_for("admin_panel"))
...
def toggle_item(kind, id):
    ...
    if not model: return redirect(url_for("admin_panel"))
...
def send_project():
    ...
    if file and file.filename:
        ...
        saved_name = save_upload(
            ...
            {"png", "jpg", "jpeg", "gif", "webp", "mp4", "webm", "mov", "mp3", "wav", "ogg", "m4a", "zip", "rar", "7z", "pdf", "txt", "doc", "docx"}
        )
        if saved_name:
